...
| 8 | Screw M5 cheese head | thread-forming, into the rail channels (4 front, 4 rear) | — |
...
| M5 fixing screws | 5.3 | x 27.8 / 454.8, y 4.9 / 83.2 — **cheese head** |
...
## Fixing screws: cheese head, not countersunk

The rail screw channels sit 5 mm from the rail edge and the panel is 0.2 mm shorter than
the rail, so the hole centres are only **4.9 mm from the panel edge**. What was evaluated:
...
| M5 countersunk DIN 965 / ISO 7046 (cross recess) | 9.2 | +0.30 mm — edge far too thin |
| **M5 cheese head DIN 912 — chosen** | **8.5** | **+0.65 mm** |

A DIN 965 countersink (cone Ø9.2 at 90°, 2.15 mm deep) was built and works in Front Panel
Designer, but 0.30 mm of remaining material at the panel edge is not enough. The panel
therefore keeps a plain Ø5.3 hole and cheese head screws.

To switch it back on, replace `CSK_TYPE = None` in `fs1a_panel.py` with the commented line
above it — hole, countersink, drill schedule, preview and STEP all follow.
